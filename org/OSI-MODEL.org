...
Phase 30 tightens the base OSI interpretation: layers 1-7 are projection
regimes over deterministic Phase 28 replay, not independent execution
machines.
...
- Must not: mutate lower-layer state, Γ, CORE, or META

** Adapter Declarations
| Adapter     | Direction      | Invariants              | Losses               | Feeds Layer |
|-------------+----------------+-------------------------+----------------------+-------------|
| tree-sitter | text → AST     | structure, byte ranges  | comments, whitespace | L10         |
| OMI-DL      | graph → logic  | facts, fixed point      | proof trees, order   | L7          |
...
** OMI-DL Integration
OMI-DL is the Datalog/Y-Z fixed-point adapter for declarative logic over the
S-P-O graph. It replaces the earlier Prolog direction with finite,
