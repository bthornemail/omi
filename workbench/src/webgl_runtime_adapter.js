(function (root, factory) {
  ...
  root.OMIWebGLRuntimeAdapter = api;
  ...
  function drawPlan(command, index, topology, drawKind) {
    return {
      kind: "DRAW_PLAN",
      draw_kind: drawKind,
      topology: topology,
      ...
      metadata: preserveMetadata(command),
      witness: fnv1a((command.draw_id || command.attachment_id || drawKind) + "|" + topology + "|" + index)
    };
  }

  function attachmentPlan(command, index) {
    return {
      kind: "ATTACHMENT_PLAN",
  ...
  function topologyFor(mode, command) {
  ...
  function adapt(stream, options) {
    ...
    stream.commands.forEach(function (command, index) {
      ...
      if (command.kind === "DRAW_POLYFORM") {
        plan.draw_calls.push(drawPlan(command, index, topologyFor(mode, command), "polyform"));
        return;
      }
      if (command.kind === "DRAW_CLOSURE_EDGE") {
        plan.draw_calls.push(drawPlan(command, index, topologyFor(mode, command), "closure-edge"));
        return;
      }
      if (command.kind === "DRAW_SCOPE_EDGE") {
        plan.draw_calls.push(drawPlan(command, index, topologyFor(mode, command), "scope-edge"));
        return;
      }
