...
ISO="${1:-build/omi.iso}"
OUT="${2:-build/qemu_foundation.log}"
...
require_line() {
    if ! grep -Fx "$1" "$NORM" >/dev/null; then
        echo "missing expected QEMU foundation line:" >&2
        echo "$1" >&2
...
if [ "$BOOT_WITNESS" != "$RECOMPUTED_WITNESS" ]; then
    echo "polyform witness mismatch:" >&2
    echo "boot=$BOOT_WITNESS recomputed=$RECOMPUTED_WITNESS" >&2
    echo "captured output: $OUT" >&2
    exit 1
fi

echo "QEMU foundation runtime vectors and polyform block verified: $OUT"
